...
@ExtendWith(MockitoExtension.class) class BankAccountServiceTest {
    @InjectMocks BankAccountService bankAccountService;

    @Mock private IBankAccountRepository iBankAccountRepository;

    @Test public void should_throw_idException_when_id_is_null() {
        Exception exception = assertThrows(IdException.class, () -> {
            bankAccountService.getBankAccount(null);
        });
    ...
    @Test public void should_throw_bankaccountfoundexception_when_account_not_found() {
        ...
        Exception exception = assertThrows(BankAccountNotFoundException.class, () -> {
            bankAccountService.getBankAccount(5500L);
        });
    ...
    @Test public void should_return_bankaccount() {
    ...
    @Test public void should_not_withdraw_when_amount_is_less_the_balance_in_the_account() {
        BankAccount bankAccount = new BankAccount(new BigDecimal(200), Collections.emptySet());
        when(iBankAccountRepository.findById(5500L)).thenReturn(Optional.of(bankAccount));
    ...
    @Test public void should_withdraw_when_account_found_and_amount_is_more_than_balance() {
    ...
    @Test public void should_deposit_amount_in_account() {
        BankAccount bankAccount = new BankAccount(new BigDecimal(2000), Collections.emptySet());
        when(iBankAccountRepository.findById(5500L)).thenReturn(Optional.of(bankAccount));
    ...
    @Test public void should_return_list_operations() {
        BankAccount bankAccount = new BankAccount(new BigDecimal(2000), Collections.emptySet());
        Set<Operation> mockOperationSet = new HashSet<Operation>();
        mockOperationSet.add(
                new Operation(bankAccount, BigDecimal.valueOf(200), LocalDate.now(), OperationType.DEPOSIT));
        mockOperationSet.add(new Operation(bankAccount, BigDecimal.valueOf(1400), LocalDate.now().minusDays(2),
                OperationType.DEPOSIT));
        mockOperationSet.add(new Operation(bankAccount, BigDecimal.valueOf(700), LocalDate.now().plusDays(3),
